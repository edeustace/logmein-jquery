  ...
  this.com.ee.LogMeIn = (function() {
    ...
    */
    ...
    function LogMeIn(element, config) {
      var defaultConfig, getUid, getUsernameValueFromTemplate, template, uid;
      uid = Math.floor(Math.random() * 10000);
      console.log(uid);
      this.ENTER = 13;
      this.TAB = 9;
      template = "<input id='username' type=\"text\" value=\"username\"></input>\n<input id='password' type=\"password\" value=\"password\"></input>";
      ...
      getUsernameValueFromTemplate = function(template) {
        var matches;
        matches = template.match(/type="text" value="(.*?)"/);
        return matches[1];
      };
      this.usernameValue = getUsernameValueFromTemplate(this.config.template);
      console.log("username value: " + this.usernameValue);
    ...
    LogMeIn.prototype.addInputs = function() {
      ...
      $username.click(function() {
        if ($(this).attr('value') === this.usernameValue) {
          $(this).attr('value', '');
        }
        return null;
